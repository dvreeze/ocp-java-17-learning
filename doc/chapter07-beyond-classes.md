...
* There is by default a generated *public canonical constructor*, setting all record component fields (but we can write custom constructor code ourselves)
...
The *canonical long constructor* (setting all instance fields) is normally *generated* by the compiler. We can define it
ourselves, however. If we do so, with the same constructor signature, no constructor will be generated by the compiler.
...
Put differently, there is *exactly one canonical record constructor*, either *implicitly* provided by the compiler or
*explicitly* provided by the programmer, and this canonical constructor is:
* either the *long constructor*, setting all `private final` instance fields
* or an explicitly provided *compact constructor* (without parameter list, even without the parentheses)
...
* A *sealed interface or class hierarchy* is a *fixed type (inheritance) hierarchy* with the sealed interface/class at the top (except for the occurrence of `non-sealed` subtypes)
  * Each *sealed interface or class* only *permits* a *fixed set of direct subtypes*
  * Each *sealed interface or class* demands those *direct subtypes* to declare themselves `final`, `sealed` or `non-sealed`
