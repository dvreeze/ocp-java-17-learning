...
/**
 * Example showing the effect of method "ExecutorService.awaitTermination".
 * <p>
...
public class AwaitTerminationExample {
    ...
    public static void main(String[] args) {
        ...
        // Without the "awaitTermination" call below, the main thread would end before the tasks complete on the other thread.
        // Still, the other thread is not a daemon thread, so it would finish the submitted tasks before the program ends.
        // In that sense, "awaitTermination" is not needed here, and "fire-and-forget" is good enough.

        try {
            boolean terminated = taskRunner.awaitTermination(10, TimeUnit.SECONDS);
            System.out.printf(addThreadToMessage("Task runner is terminated (directly after \"awaitTermination\"): %b%n"), terminated);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        System.out.printf(addThreadToMessage("Task runner is terminated (directly after \"awaitTermination\"): %b%n"), taskRunner.isTerminated());
    }
}
